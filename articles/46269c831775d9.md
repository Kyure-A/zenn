...
MCP Server についてはすでに Nix を用いて宣言的に管理する手法が存在しますが、まだ Agent Skills について管理する手法は見当たらないように思えました。非 Nix での手法としては Open Skills なるものがあるらしいです。詳しくは調べる気になりませんでした（Nix ではないので）
...
Agent Skills を全て有効にしたいリポジトリ（例えば自身が作ったもの）については `skills.enableAll` に記述すればすべてがリンクされます。Anthropics/Skills のような全部入りのものから一部のみ抜き出したい場合は、`skills.enable` にそれぞれのスキル名を記述してあげれば、それらのみがリンクされます。

## まとめ
Nix で Agent Skills を管理できると気持ちがいい
