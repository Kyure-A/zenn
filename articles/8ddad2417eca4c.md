...
## predict-on はどう使いにくいのか
書いているとかなり使いづらいプラグインである気がしてきました。
### 意図しない補完が行われる
`git c` と入力すると `git checkout master` が補完されて欲しいのに `git commit -m "fix bug"` など、意図していないコマンドが補完されることが多いです。
### 行の途中で編集すると残りが削除される場合がある
